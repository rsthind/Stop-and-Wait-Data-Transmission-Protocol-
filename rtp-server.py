...
def ceaser_cypher(message):
    """
        Spring 2021 cencryption function
    """ 
    newmessage = message
    for i in range(len(message)):
        if not message[i].isupper() and not message[i].islower():
            continue
        ch = ((ord(message[i]) - 65 + 6) % 26 + 65) \
            if message[i].isupper() \
            else ((ord(message[i]) - 97 + 6) % 26 + 97)
        newmessage = newmessage[:i] + chr(ch) + newmessage[i+1:]
    return newmessage

def checksum(msg):
    """
        Spring 2021 check sum function
    """ 
    check = 0
    i = 1
    MOD_VAL = (1 << 31) - 1
    for letter in msg:
        check = (check + i * ord(letter)) % MOD_VAL
        i += 1

    return check % MOD_VAL
...
def runServer():
    ...
    while True:
        ...
        essagemay = ceaser_cypher(message)
